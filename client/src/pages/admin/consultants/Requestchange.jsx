...
const statusLabel = () => "REQUEST CHANGES";
...
const RequestChanges = () => {
  ...
  const fetchRequestChanges = async (pageNo = 1, search = "") => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    fetchRequestChanges(page, searchText);
  }, [page, searchText]);
      ...
      <div className="flex flex-col sm:flex-row justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-400">
            Consultants who need document corrections
          </p>
        </div>
        ...
        <table className="w-full min-w-[900px]">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan="6" className="py-10 text-center text-gray-400">
                  No request-change consultants found
                </td>
              </tr>
            ) : (
              consultants.map((c) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${verificationStyles.REQUEST_CHANGES}`}
                    >
                      {statusLabel()}
                    </span>
                  </td>
